        ...
        else
        {
          ...
          for (auto pValueChild : pValueChildren)
          {
            ...
            if (tag == "name")
              ...
              {
                valueOfTextNode = pChild->children()[0]->value();elem.children().push_back(valueOfTextNode);
              }
			else if (tag == "payload")
				pl = PayLoad::fromXmlElement(pValueChild);elem.payLoad(pl);
		  }
        }
        db_[key] = elem;
      }
