...
#include <vector>
#include <iostream>
#include "../NoSqlDb (1)/XmlDocument/XmlDocument/XmlDocument.h"
#include "../NoSqlDb (1)/XmlDocument/XmlElement/XmlElement.h"
#include "../NoSqlDb (1)/DbCore/Definitions.h"
...
namespace NoSqlDb
{
	...
	*/
	...
	inline void PayLoad::showPayLoadHeaders(std::ostream& out)
	{
		...
		out << std::setw(20) << std::left << "Check-in Status";
	...
	{
		out << "\n  ";
		out << std::setw(15) << std::left << elem.name().substr(0, 15);
		out << std::setw(40) << std::left << elem.payLoad().value().substr(0, 38);
		out << std::setw(20) << std::left << elem.payLoad().status();
